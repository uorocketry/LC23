...
ax1.plot(df['adjusted_timestamp'], df['gyro_x'], label='X Axis', color='r')
ax1.set_ylabel('Gyro rad')
ax1.legend()

ax2.plot(df['adjusted_timestamp'], df['gyro_y'], label='Y Axis', color='g')
ax2.set_ylabel('Gyro rad')
ax2.legend()

ax3.plot(df['adjusted_timestamp'], df['gyro_z'], label='Z Axis', color='b')
ax3.set_xlabel('Timestamp Microseconds')
ax3.set_ylabel('Gyro rad')

plt.suptitle('Gyro Data Over Time')
plt.show()