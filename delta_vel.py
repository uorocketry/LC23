...
ax1.plot(df['adjusted_timestamp'], df['dv_x'], label='North', color='r')
ax1.set_ylabel('Delta Velocity m/s')
ax1.legend()

ax2.plot(df['adjusted_timestamp'], df['dv_y'], label='East', color='g')
ax2.set_ylabel('Delta Velocity m/s')
ax2.legend()

ax3.plot(df['adjusted_timestamp'], df['dv_z'], label='Down', color='b')
ax3.set_xlabel('Timestamp Microseconds')
ax3.set_ylabel('Delta Velocity m/s')
